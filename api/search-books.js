export default async function handler(req, res) {
  ...
  try {
    ...
    const results = (data.docs || []).slice(0, 8).map(b => ({
      ...
      // Real Open Library work key (e.g. "/works/OL12345W"), distinct per
      // book even when titles collide. Used as source_id so each search
      // result's rating state is tied to the actual work, not just title text.
      sourceId: b.key || null,
    }));
