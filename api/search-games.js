export default async function handler(req, res) {
  ...
  try {
    ...
    const results = (data.results || []).slice(0, 8).map(g => ({
      ...
      // Real RAWG ID, distinct per game even when titles collide (e.g. two
      // different games both named "Foundation"). Used as source_id so
      // each search result's rating state is tied to the actual thing,
      // not just its title text.
      sourceId: g.id ? String(g.id) : null,
    }));
