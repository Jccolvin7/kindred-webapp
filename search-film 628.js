  ...
  try {
    ...
    const results = [
      ...(movies.results || []).slice(0, 6).map(m => ({
        ...
        // TMDB movie and TV ids are each unique only WITHIN their own type
        // (a movie id 550 and a tv show id 550 can both exist), so this is
        // prefixed by type to keep them from ever colliding with each other.
        sourceId: m.id ? `movie-${m.id}` : null,
      })),
